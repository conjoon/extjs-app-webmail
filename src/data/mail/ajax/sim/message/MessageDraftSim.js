...
Ext.define('conjoon.cn_mail.data.mail.ajax.sim.message.MessageDraftSim', {
    ...
    Ext.ux.ajax.SimManager.register({
        ...
        doDelete : function(ctx) {
            ...
            if (!key.id || !key.mailFolderId || !key.mailAccountId) {
                console.log("DELETE MessageDraft - no numeric id specified.");
                return {success : false};
            }

            console.log("DELETE MessageDraft - ", key);
            ...
            for (var i = messageItems.length - 1; i >= 0; i --) {
                if (messageItems[i].mailAccountId === key.mailAccountId &&
                    messageItems[i].mailFolderId === key.mailFolderId &&
                    messageItems[i].id === key.id) {
